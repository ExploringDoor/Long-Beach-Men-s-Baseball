...
- **Home "Upcoming Matchups" fixed too** — it used its own `parseSchedLabel` (same `new Date(label+" 2026")` bug), so the 9/26 games parsed as epoch and it fell back to showing yesterday's championship. Routed `parseSchedLabel`, the week-scores date filter, and the TWIB date formatter through `toISODate`.
